...
class ShowPuzzles extends Component {
    ...
    handleChangeSolution = (e, i) => {
        ...
        let puzzleCopy = [...this.state.solution];
        puzzleCopy[+e.target.name][i] = (e.target.value * 1);
        this.setState({ solution: puzzleCopy }, () => {
            console.log('finished')
        })
        // this.setState({ solution: 'asdf' })
    }
    handleSubmit = (puzzle) => {
        console.log(this.validSolution(puzzle))
    }
    ...
    render() {
        ...
        return (
            ...
            <div>
                <Board board={puzzle.board} solution={solution} handleChangeSolution={this.handleChangeSolution} id={puzzle.id} key={puzzle.id} />
                <button onClick={this.handleClick}>Check Solution!</button>
            </div>
        )
    }
}
